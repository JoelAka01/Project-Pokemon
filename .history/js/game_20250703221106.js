...
export class Game {
   ...
   checkAttackSelectionAfterReload() {
      ...
      console.log("🔄 Vérification de l'état de combat après rechargement:", {
         playerCard: playerCard ? playerCard.name : "AUCUNE",
         opponentCard: opponentCard ? opponentCard.name : "AUCUNE",
      ...
      // Conditions pour demander la sélection d'attaque :
      // 1. Le joueur a une carte active avec des attaques
      // 2. L'adversaire a une carte active 
      // 3. Le joueur n'a pas encore sélectionné d'attaque
      // 4. Nous sommes dans une phase de combat ou la sélection d'attaque a commencé
      // 5. L'un des deux Pokémon a des PV initialisés (indique qu'un combat était en cours)
      if (playerCard && 
          playerCard.attacks && 
          playerCard.attacks.length > 0 && 
          opponentCard && 
          !this.battleSystem.selectedPlayerAttack &&
          (this.battleSystem.battlePhase === 'selecting-attacks' || 
           this.battleSystem.attackSelectionStarted || 
           this.battleSystem.playerHP !== null || 
           this.battleSystem.opponentHP !== null)) {
         
         console.log("✅ Combat en cours détecté après rechargement - Demande de sélection d'attaque");
         
         // Délai pour laisser le temps à l'UI de se charger
         setTimeout(() => {
            this.battleSystem.showAttackChoiceModal(playerCard, true);
         }, 1000);
      }
   }
