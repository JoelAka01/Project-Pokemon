export class BattleSystem {
   ...
   cleanup() {
      ['battle-attack-display', 'change-card-modal', 'battle-end-modal', 'attack-choice-modal',
         ...
         'opponent-card-selection-animation', 'opponent-attack-display']
         .forEach(id => {
   ...
   async handleOpponentKOWithDelay() {
      ...
      if (this.game.opponent.hand.cards.length > 0) {
         // Afficher l'animation de sélection de carte
         this.showOpponentCardSelectionAnimation();

         // Attendre que l'animation se termine (4 secondes)
         await this.delay(4000);

         // Supprimer l'animation
         this.hideOpponentCardSelectionAnimation();
   ...
   showOpponentCardSelectionAnimation() {
      // Supprimer l'animation existante si elle existe
      this.hideOpponentCardSelectionAnimation();

      const animation = document.createElement('div');
      animation.id = 'opponent-card-selection-animation';
      animation.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';

      animation.innerHTML = `
         <div class="bg-white rounded-lg p-8 max-w-md mx-4 text-center">
            <h3 class="text-xl font-bold mb-6 text-gray-800">🤖 Adversaire</h3>
            <div class="mb-6">
               <div class="animate-pulse flex justify-center space-x-2 mb-4">
                  <div class="w-16 h-24 bg-blue-300 rounded-lg"></div>
                  <div class="w-16 h-24 bg-blue-400 rounded-lg"></div>
                  <div class="w-16 h-24 bg-blue-300 rounded-lg"></div>
               </div>
               <div class="text-lg text-gray-600 mb-2">Choisit une carte...</div>
   ...
   hideOpponentCardSelectionAnimation() {
      const animation = document.getElementById('opponent-card-selection-animation');
      if (animation) {
         animation.remove();
      }
   }
