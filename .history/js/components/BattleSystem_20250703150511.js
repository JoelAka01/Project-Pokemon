export class BattleSystem {
   ...
   async handleOpponentKO() {
      if (this.game.opponent.hand.cards.length > 0) {
         ...
         try {
            if (this.game.dragAndDrop && this.game.dragAndDrop.autoPlayOpponent) {
               this.game.dragAndDrop.autoPlayOpponent();
               ...
               console.error("❌ Impossible d'accéder à la méthode autoPlayOpponent");
               const notification = document.getElementById('opponent-replacement-notification');
               if (notification) notification.remove();
            }
         } catch (error) {
