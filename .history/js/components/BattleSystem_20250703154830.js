export class BattleSystem {
   ...
   showAttackChoiceModal(card, isPlayer = true) {
    ...
    cleanup() {
      ...
      // Nettoyer aussi les notifications sans ID spécifique
      document.querySelectorAll('.fixed.top-4.left-1\\/2').forEach(notification => {
         if (notification.textContent.includes("L'adversaire a placé")) {
            notification.remove();
         }
      });
   }eturn;
   ...
   async handleKOsAfterBattle() {
      ...
      if (playerWasKO && opponentWasKO) {
         ...
         // Seul l'adversaire est KO - remplacement automatique immédiat
         console.log("🤖 Adversaire KO - placement automatique immédiat");
         
         // Forcer le placement d'une nouvelle carte immédiatement
         if (this.game.opponent.hand.cards.length > 0) {
            console.log("🤖 Cartes disponibles, placement automatique...");
            const success = this.forceOpponentCardPlacement();
            if (success) {
               console.log("✅ Carte adverse placée avec succès");
               // Continuer le combat après un délai
               setTimeout(async () => {
                  await this.handleOpponentKO();
               }, 1000);
            } else {
               console.error("❌ Échec du placement automatique");
            }
         } else {
