export class BattleSystem {
   ...
   cleanup() {
      ['battle-attack-display', 'change-card-modal', 'battle-end-modal', 'attack-choice-modal',
         ...
         'player-replacement-notification', 'opponent-attack-display']
         .forEach(id => {
   ...
   async performOpponentAttack() {
      ...
      // Si le joueur n'a pas encore choisi d'attaque, choisir automatiquement
      if (!this.selectedPlayerAttack && this.game.player.activeCard && this.game.player.activeCard.attacks) {
         const playerAttacks = this.game.player.activeCard.attacks;
         if (playerAttacks.length > 0) {
            // Choisir automatiquement la première attaque du joueur
            this.selectedPlayerAttack = playerAttacks[0];
            console.log("🎯 Attaque joueur auto-sélectionnée:", this.selectedPlayerAttack.name);
         }
      }

      if (!this.selectedPlayerAttack) {
         console.log("❌ Impossible de sélectionner une attaque pour le joueur");
         return;
      }

      console.log("🎯 Attaque joueur:", this.selectedPlayerAttack.name);

      // Maintenant lancer le combat complet avec les deux attaques
      this.opponentAttacksFirst = true; // L'adversaire attaque en premier après avoir placé sa carte
   ...
   async handleDoubleKO() {
      console.log("🔄 Gestion du double KO - Début de la séquence");

      // Afficher la notification pour l'adversaire
      this.showOpponentReplacementNotification();

      // Attendre 3 secondes avec la notification
      await this.delay(3000);

      // L'adversaire place sa carte automatiquement
      if (this.game.opponent.hand.cards.length > 0) {
         console.log("🤖 L'adversaire place sa carte automatiquement...");

         // Placement direct de la carte adverse
         const randomIndex = Math.floor(Math.random() * this.game.opponent.hand.cards.length);
         const selectedCard = this.game.opponent.hand.cards.splice(randomIndex, 1)[0];
         this.game.opponent.activeCard = selectedCard;
         this.game.opponentActiveZone.setActiveCard(selectedCard);
         ...
         // Notification de carte adverse placée
         this.showOpponentCardPlacedNotification(selectedCard.name);
         ...
         // Attendre un peu puis demander au joueur de poser sa carte
         await this.delay(1000);

         // S'assurer que l'adversaire attaque en premier après le double KO
         this.opponentAttacksFirst = true;
         this.resetSelectedAttacks();

         console.log("🎯 Maintenant c'est au tour du joueur de poser sa carte");
         this.showPlayerReplacementNotification();

      } else {
         console.log("❌ L'adversaire n'a pas de carte à poser");
         this.showOpponentNoCardsNotification();

         // Même si l'adversaire n'a pas de carte, le joueur doit poser la sienne
         await this.delay(1000);

         // Dans ce cas, le joueur attaque en premier car l'adversaire n'a pas de carte
         this.opponentAttacksFirst = false;
         this.resetSelectedAttacks();
